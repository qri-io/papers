...
Finally, if the `Structure` changes, the `Data` necessarily changes as well, which means the new `Dataset` object points to new `Structure` and `Data` objects.  This indicates that the underlying data set has changed in such a way---either in structure or in values---that its hash signature is no longer the same.
...
```json
{
  ...
  "statement" : "SELECT a.col_0, a.col_1, a.col_2, a.col_3 FROM a WHERE a.precip > 2.0",
  "outputStructure" : "1224c9bb19b441c0303d872832d3fd5066f7960b6ab7624b9e043969e2ba7181e834"
}
```

## Query execution

Excuting a query is conceptually simple, and requires only a consistent procedure for the process.  A query engine can proceed roughly as follows:

1. A user's database query is obtained from an external mechanism (such as a user interface, or another software system).  The concrete, specific query expression and any data that it refers to are used to create an initial `Dataset` object with references to `Data` hashes as needed.
...
# Distributed queries, linked data

Schemes for linking distributed data sets also exist, particularly in the form of Linked Data [@heath2011linked], but the methods of querying and accessing data currently involve database access and retrieval. With the approach outlined here, the result of any operation upon a dataset is a structure that can be utilized by any number of outside tools capable of parsing widely-adopted formats. It is an explicit design goal to present natural interoperability. It should be possible to traverse all recorded operations without the use of any database technology.

By forcing the database to output to a standard format, the version or entire applicaiton that operates on a dataset can change at any step in the history of changes to a dataset, so long as it maintains the key linked-data characteristics. This makes it possible to chain use of different tools together that operate on the same graph of datasets, selecting the most appropriate tool for a given task. These common formats also provide a degree of future-proofing, allowing the results of using a tool to be utilized long after the usefulness of the tool itself has expired.

The first step toward realizing the value of this interoperability will be to add support for the InterPlanetary Linked Data specification, IPLD [@ipld2017]. Through use of an IPLD _adapter_ it will be possible to coordinate query results with other linked data structures like blockchain-based cryptocurrencies or git version histories. This data-format bridging opens up possilibities like creating smart contracts on data procuring & fullfillment (ehterium-qri linking), or syncing formal data schemas & test data with source code (git-qri linking).


## Metadata Interoporability

Aside from the caveat of using content-addressed identifiers instead of urls, it is an express design goal that first-order metadata be collected with an eye toward supporting existing metadata formats, deriving as many hard-earned lessons as possible from the Collections, Libraries, Archives & Muesum communities (CLAM). Datasets can and should support arbitrary metadata, but at the same time must place restrictions on the use of reserved keywords. These restrictions will be designed to maximize the potential to transpose dataset metadata into a number of existing archival metadata formats, including the Data Catalog vocabulary [@dcat2014], the open knowledge foundation datapackage format, schema.org, and the CKAN/DKAN, dataverse apis. by making use of same-term metata wherever possible, and placing restrictions on fields that intersect & cover the minimums of these formats, we leave open the potential to write libraries that transpose dataset data between all of these specifications.
