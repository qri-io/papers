...
This "database in the middle" pattern is the result of serving the needs _application data_, or data who's main purpose is to serve as a structured store of state for software. When referring to Open data, we're often talking about _research data_, that is, data who's primary purpose is to be mined & probed for insights. There is no doubt that many types of data can be both application and research data, but the vast majority of databases are built with a favouritism for application data, prioritizing speed & efficency of query execution, with little-to-no concern for what is done with the results of a query.
