...
A fundamental goal of deterministic querying is to _maximize hash collisions_ in content-addressed networks. The techniques below are aimed to ensure that datasets contain characteristics that resolve to equal hashes, creating a natural link with datasets thas share these characteristics. as well as semantically-equivalent queries for datasets, each resolve to the same hash even if created by different parties in different locations at different times. These shared characteristics form a common lexicon which peers in a distributed network can use to compare datasets, forming an internet-scale graph.
...
Queries written in SQL make reference to columns in a tabular database, and express conditions that values must satisfy to be selected. Here is an example from the `precip` dataset referenced above:
...
So the question stands, why go to all this effort calculating abstract queries and structures? The advantage of using these abstract forms becomes clear when isolated & placed on a content-addressed network. At time of addition, we separate out the abstract structure as independent files & write each to the network, which gives us a concrete hash that we store in `Dataset` as a reference. For example, here is the above output dataset in it's referenced form:
...
Instead of storing the abstract structures itself in the dataset, we store the _reference to it's hash_.
